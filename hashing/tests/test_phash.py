"""
Pytest unit tests for the pHash algorithm.
...
_hasher = PerceptualHash()
compute = _hasher.compute
hamming_distance = PerceptualHash.hamming_distance
similarity = PerceptualHash.similarity
...
class TestCompute:
    ...
    def test_hash_length_is_63(self, gradient_image):
        h = compute(gradient_image)
        assert len(h) == 63, f"Expected 63 bits (64 minus DC), got {len(h)}"
...
class TestHammingDistance:
    ...
    def test_completely_opposite_returns_full_length(self):
        assert hamming_distance("1" * 63, "0" * 63) == 63
...
class TestSimilarity:
    ...
    def test_opposite_returns_zero(self):
        assert similarity("1" * 63, "0" * 63) == 0.0
    ...
    def test_score_consistent_with_distance(self):
        h1 = "1" * 63
        h2 = "0" * 10 + "1" * 53  # distance = 10
        assert similarity(h1, h2) == pytest.approx(1.0 - 10 / 63)
...
class TestRobustness:
    """
    Verify that pHash produces low Hamming distance for common
