...
from hashing.algorithms.Chash import ColorHash
...
def benchmark_own(own_folder):
    ...
    with open(csv_path, "w") as f:
        ...
        for reference in tqdm.tqdm(ref_files, desc="Processing Own Dataset"):
            ...
            img1 = ColorHash().compute(ref_path)
            ...
            if os.path.exists(variant_subfolder):
                for altered in os.listdir(variant_subfolder):
                    ...
                    img2 = ColorHash().compute(os.path.join(variant_subfolder, altered))
                    
                    hamming_distance = ColorHash.hamming_distance(img1, img2)
                    similarity = ColorHash.similarity(img1, img2)
...
def benchmark_disc21(disc21_folder):
    ...
    def get_hash(path):
        if path not in hash_cache:
            hash_cache[path] = hasher.compute(path)
        ...
        with open(csv_ground_truth, "r") as gt:
            ...
            for line in tqdm.tqdm(lines[1:], desc="Processing DISC21", leave=True):
                ...
                try:
                    ...
                    h_dist = ColorHash.hamming_distance(img1, img2)
                    sim = ColorHash.similarity(img1, img2)
