"""
Benchmark sHash: find the Hamming-distance threshold that maximises
accuracy / F1 on the variants.csv dataset.

Two ways to run from the hashing/ directory:

    # Standalone script (prints full sweep table)
    python benchmark_shash.py

    # Pytest mode (assertions on the calibrated values; -s shows the table)
    pytest benchmark_shash.py -v -s
"""
...
from algorithms.shash import compute, hamming_distance
...
# Calibrated against the current own/ dataset.
EXPECTED_BEST_F1_THRESHOLD = 29
EXPECTED_FULL_RECALL_THRESHOLD = 31
...
def run_sweep():
    ...
    return {
        ...
        "results": results,
        "best_accuracy": max(results, key=lambda r: (r["accuracy"], r["f1"])),
...
def print_full_report(sweep):
    positives = sweep["positives"]
    negatives = sweep["negatives"]
    positive_distances = sweep["positive_distances"]
    negative_distances = sweep["negative_distances"]
    results = sweep["results"]
    best_accuracy = sweep["best_accuracy"]
    best_f1 = sweep["best_f1"]

    print("=" * 78)
    print("sHash Benchmark - Hamming Threshold Sweep")
    ...
    print_summary(
        ...
        results[sweep["full_recall_threshold"]],
    )
...
def test_dataset_files_present():
    assert VARIANTS_CSV.exists(), f"Missing dataset: {VARIANTS_CSV}"


def test_pair_counts(sweep):
    assert len(sweep["positives"]) == 54
    assert len(sweep["negatives"]) == 111


def test_distance_separation(sweep):
    # Positives must be clearly closer than negatives or the hash is useless.
    assert sweep["positive_distances"].mean() < sweep["negative_distances"].mean()
    assert sweep["positive_distances"].max() < HASH_BITS


def test_best_f1_threshold_is_stable(sweep):
    assert sweep["best_f1"]["threshold"] == EXPECTED_BEST_F1_THRESHOLD
    assert sweep["best_f1"]["tp"] == 50
    assert sweep["best_f1"]["fp"] == 1
    assert sweep["best_f1"]["recall"] == pytest.approx(50 / 54)
    assert sweep["best_f1"]["precision"] == pytest.approx(50 / 51)


def test_full_recall_threshold_is_stable(sweep):
    full_recall = sweep["results"][sweep["full_recall_threshold"]]
    assert sweep["full_recall_threshold"] == EXPECTED_FULL_RECALL_THRESHOLD
    assert full_recall["recall"] == 1.0
    assert full_recall["fp"] == 6
    assert sweep["best_f1"]["f1"] > full_recall["f1"]


def test_print_report(sweep, capsys):
    # Run the full report so `pytest -s` shows the same output as the script.
    with capsys.disabled():
        print_full_report(sweep)
