...
All logic lives on the HSVColorHash class so it shares the common ImageHasher
interface (compute / hamming_distance / similarity). The lower-level numeric
helpers (rgb_to_hsv, rgb_to_intensity, hsv_hash, hamming, hash_to_hex) are
exposed as static/class methods on HSVColorHash.
"""
...
class HSVColorHash(ImageHasher):
    """HSV color-distribution hash exposed through the ImageHasher interface."""

    def __init__(self, binbits=3):
        self.binbits = binbits
        self.HASH_BITS = 14 * binbits  # 42 with binbits=3

    # -- ImageHasher interface ----------------------------------------------

    def compute(self, image_input):
        """Hash a file path or PIL image (loaded as RGB) into a binary string."""
        rgb = np.asarray(self._to_pil(image_input).convert("RGB"))
        return self._to_bitstring(self.hsv_hash(rgb, self.binbits))
    ...
    def hamming(hash1, hash2):
        """Hamming distance between two HSVHash bit arrays (numpy)."""
